...
function createBlogPostSlide(post) {
    ...
    slide.innerHTML = `
        ...
        <p>${post.author || 'Unknown Author'} - ${post.published_at ? new Date(post.published_at).toLocaleDateString() : 'Unknown Date'}</p>
        <img src="${post.image_url || '/images/fallback-image.png'}" alt="${post.title || 'No Title'}" onerror="this.onerror=null;this.src='/images/fallback-image.png';" />
        <p class="summary">${post.summary || 'No summary available'}</p>
    `;
    return slide;
}
...
function createBlogPostCard(post) {
    ...
    card.innerHTML = `
        <div class="thumbnail"><img src="${post.image_url || '/images/fallback-image.png'}" alt="${post.title || 'No Title'}" onerror="this.onerror=null;this.src='/images/fallback-image.png';"></div>
        <div class="post-info">
...
async function fetchBlogPosts() {
    try {
        const response = await doFetch("GET", "https://v2.api.noroff.dev/blog/posts/ItDah");
